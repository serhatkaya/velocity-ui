...
export class VelocityStepperComponent implements AfterContentInit {
  protected destroy$ = new Subject();
  @Input() navigation: boolean = true;
  @Input() enableGoBack: boolean = true;
  @ContentChildren(VelocityStepComponent)
  steps!: QueryList<VelocityStepComponent>;
  ...
  public setActiveStepById(id: string) {
    ...
    if (!step) {
      console.warn(`[VelocityStepperComponent] Step with id "${id}" not found`);
      return;
    }
